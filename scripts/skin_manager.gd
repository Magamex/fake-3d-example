...
@export var skin: Resource; #variable for the skin you want to use
@export var map: Resource; #variable for the UV map for your character
...
func update_skin(texture):
	...
	skin = texture
	if texture is CompressedTexture2D: #check if it's a stream texture, if it's not, doesn't need to load the image
...
func read_folder(path): #funcion for reading a folder and retrieve it's files (png only but you can change for anything you'd like)
	...
	var dir = DirAccess.open(path) #directory
	#dir.open(path) #open the directory 
	dir.list_dir_begin()  #starts listing all the files inside# TODOConverter3To4 fill missing arguments https://github.com/godotengine/godot/pull/40547
	while true: #while still has files inside the folder
		...
		if file == "": #if there's no more files
			...
			if file.ends_with(".png"): #if the files ends with super.png (you can change by any extension you'd like)
